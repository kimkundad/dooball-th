...
@section('title', 'แก้ไข API')

@section('content')
    @section('breadcrumb')
        <li><a href="{{ URL::to('/') }}/admin/settings/api-football"><span class="icon icon-beaker"></span>API ทั้งหมด</a></li>
        <li><a onclick="Javascript:void(0);"><span class="icon icon-double-angle-right"></span>แก้ไข API</a></li>
    @endsection
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <form role="form" id="api_football_form">
                                    @csrf
                                    <input type="hidden" name="api_id" id="api_id" value="{{ $form->id }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                                            <input type="text" class="form-control" name="api_url" id="api_url" value="{{ $form->api_url }}" maxlength="125" />
                                        </div>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                                            <input type="text" class="form-control" name="api_desc" id="api_desc" value="{{ $form->api_desc }}" maxlength="200" />
                                        </div>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                                            <input type="number" class="form-control" name="params_count" id="params_count" value="{{ $form->params_count }}" min="0" max="5" />
                                        </div>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group mb-2">
                                        ...
                                        <div class="col-lg-10 col-md-10 col-sm-6 col-xs-12">
                                            ...
                                            <div class="d-flex api-params-type mb-2">
                                                @if((int) $form->params_count > 0)
                                                    @php
                                                        $arrTypes = explode('/', $form->params_type);
                                                    @endphp
                                                    @for($i = 0; $i < (int) $form->params_count; $i++)
                                                        <input type="text" class="form-control params-data" name="params_types[]" id="params_type_{{ $i }}" value="{{ $arrTypes[$i] }}" maxlength="125" />
                                                    @endfor
                                                @endif
                                            </div>

                                            <p class="mb-0">ตัวอย่าง</p>
                                            <div class="d-flex api-params-mask">
                                                @if((int) $form->params_count > 0)
                                                    @php
                                                        $arrMasks = explode('/', $form->params_mask);
                                                    @endphp
                                                    @for($i = 0; $i < (int) $form->params_count; $i++)
                                                        <input type="text" class="form-control params-data" name="params_masks[]" id="params_mask_{{ $i }}" value="{{ $arrMasks[$i] }}" maxlength="125" />
                                                    @endfor
                                                @endif
                                            </div>
                                        </div>
        ...
        function submitForm(this_form) {
            Swal.fire({
                ...
                onOpen: () => {
                    ...
                    $.ajax({
                        url: $('#base_url').val() + '/api/admin/api-football/save-update',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',
                        cache: false,
                        ...
                        success: function (response) {
                            ...
                            setTimeout(function () {
                                // window.history.back();
                                window.location = $('#base_url').val() + '/admin/settings/api-football';
                            }, 2000);
